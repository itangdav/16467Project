...
# Counters for cheating
cheat_counters = [3,3,6]
...
def playPaper():
    global PLAYER_SCORE, NICO_SCORE
    disable_buttons()
    (display_move, nico_move) = get_move(1)
...
def playScissor():
    global PLAYER_SCORE, NICO_SCORE
    disable_buttons()
    (display_move, nico_move) = get_move(2)
...
def get_move(other_move):
    nico_move = robot_moves[getCurrRoundNum()-1]
    index = 0
    while(cheat_counters[index] == -1) and (index < 3):
        index += 1
    
    if index == 3:
        return (computer_value[str(nico_move)],computer_value[str(nico_move)])

    cheat_counters[index] -= 1 
    if(cheat_counters[index] != -1):
        return (computer_value[str(nico_move)],computer_value[str(nico_move)])
    else:
        if (nico_move + 3 - other_move) % 3 == 1:
            cheat_counters[index] += 1 
            return (computer_value[str(nico_move)],computer_value[str(nico_move)])
        else:
            # Return display move and theoretical move
            return (computer_value[str(nico_move)],computer_value[str((other_move+1)%3)])
